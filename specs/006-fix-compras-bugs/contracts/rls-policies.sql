...
WITH CHECK (
  EXISTS (
    ...
    INNER JOIN public.departments d ON ur.department_id = d.id
...
-- List all policies on ticket_quotations:
-- SELECT * FROM pg_policies WHERE tablename = 'ticket_quotations';

-- Test INSERT permission for a specific user:
-- SELECT EXISTS (
--   SELECT 1
--   FROM public.user_roles ur
--   INNER JOIN public.roles r ON ur.role_id = r.id
