...
test.describe('BUG-012: Gerente triar chamados', () => {
  test('Gerente de Operações deve conseguir triar chamado de Compras', async ({ page }) => {
    // 1. Login como admin
    ...
    const ticketRow = page.locator('table tbody tr, [role="table"] tbody tr').filter({
    ...
    const triageButton = page.locator('button:has-text("Fazer Triagem")').or(
      page.locator('button:has-text("Triagem")')
    )
    
    // Verificar status atual do ticket
    const statusText = await page.locator('text=/Aguardando Triagem/i').textContent().catch(() => null)
    
    if (statusText || await page.locator('text=/awaiting_triage/i').count() > 0) {
      ...
      await expect(page.locator('text=/Prioridade/i').or(page.locator('text=/priority/i'))).toBeVisible({ timeout: 5000 })
      
      // 9. Preencher campos de triagem
      // Selecionar prioridade
      const prioritySelect = page.locator('select[name="priority"]').or(
        page.locator('[name="priority"]')
      )
      ...
      if (await prioritySelect.count() > 0) {
        ...
        if (await priorityLabel.count() > 0) {
          const priorityField = priorityLabel.locator('..').locator('select, [role="combobox"]').first()
      ...
      const assignedSelect = page.locator('select[name="assigned_to"]').or(
        page.locator('[name="assigned_to"]')
      )
      ...
      const saveButton = page.locator('button:has-text("Salvar")').or(
        page.locator('button:has-text("Confirmar")').or(
      ...
      await expect(page.locator('text=/triagem/i').or(page.locator('text=/sucesso/i'))).toBeVisible({ timeout: 5000 })
      
      // 12. Verificar que dialog foi fechado
      await expect(page.locator('text=/Prioridade/i')).not.toBeVisible({ timeout: 3000 })
    } else {
      // Se o ticket não está em "Aguardando Triagem", apenas verificar que a página carregou
      console.log('Ticket não está em status "Aguardando Triagem". Teste parcial: página de detalhes carregou corretamente.')
    }
  })
  
  test('Gerente de Operações deve conseguir triar chamado de Manutenção', async ({ page }) => {
