...
test.describe("BUG-014: Gerente fechar chamados", () => {
  test("Gerente de Operações deve conseguir fechar chamado de Compras", async ({
    page,
  }) => {
    // 1. Login como admin
    ...
    const currentStatus = await page
      ...
      .catch(() => null);
    const isFinalStatus =
      currentStatus?.includes("Fechado") ||
    ...
    await expect(
      page
        .locator("text=/Status alterado/i")
        .or(page.locator("text=/Fechado/i"))
  ...
  test("Gerente de Operações deve conseguir fechar chamado de Manutenção", async ({
    page,
  }) => {
  ...
  test('Botão "Fechar Chamado" não deve aparecer para Supervisor', async ({
    page,
  }) => {
