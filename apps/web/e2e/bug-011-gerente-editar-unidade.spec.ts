...
const GERENTE_EMAIL = process.env.TEST_GERENTE_EMAIL || 'gerente_operacoes_teste@garageinn.com'
...
test.describe('BUG-011: Gerente editar unidades', () => {
  test.beforeEach(async ({ page }) => {
    ...
    const gerenteRow = page.locator(`table tbody tr:has-text("${GERENTE_EMAIL}")`)
  ...
  test('Gerente deve ver botão "Editar" na página de unidades', async ({ page }) => {
    ...
    const unitCards = page.locator('[data-testid="unit-card"], .grid > div').first()
    await expect(unitCards).toBeVisible()
    
    // Verificar que há botão de ações (três pontos) ou dropdown
    const actionsButton = page.locator('button:has([data-lucide="more-vertical"]), button[aria-haspopup="menu"]').first()
  ...
  test('Gerente deve conseguir acessar página de edição de uma unidade', async ({ page }) => {
    // Navegar para página de unidades
    await page.goto('/unidades')
    await page.waitForSelector('text=Unidades')
  ...
  test('Gerente deve conseguir modificar e salvar dados de uma unidade', async ({ page }) => {
    // Navegar para página de unidades
    await page.goto('/unidades')
    await page.waitForSelector('text=Unidades')
    ...
    if (await nameInput.count() > 0) {
      ...
      if (currentUrl.includes('/editar')) {
        ...
        await expect(page.locator('text=salvo, text=sucesso, text=atualizado')).toBeVisible({ timeout: 5000 })
      } else {
  ...
  test('Gerente não deve ver botão "Editar" se não tiver permissão units:update', async ({ page }) => {
    ...
    const hasActionsButton = await page.locator('button:has([data-lucide="more-vertical"]), button[aria-haspopup="menu"]').count() > 0
    const hasEditButton = await page.locator('button:has-text("Editar"), a:has-text("Editar")').count() > 0
