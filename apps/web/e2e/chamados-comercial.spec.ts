...
const COMERCIAL_USER_EMAIL =
  process.env.TEST_COMERCIAL_EMAIL || "comercial@teste.com";
const COMERCIAL_USER_PASSWORD =
  process.env.TEST_COMERCIAL_PASSWORD || "senha123";
...
test.describe("Chamados Comerciais", () => {
  ...
  test.describe("Criacao de Chamado", () => {
    ...
    test("deve exibir todos os campos do formulario", async ({ page }) => {
      ...
      await expect(
        page.locator('label:has-text("Tipo Comercial")')
      ).toBeVisible();
      ...
      await expect(
        page.locator('label:has-text("Nome / Razao Social")')
      ).toBeVisible();
      ...
      await expect(
        page.locator('label:has-text("Valor do Contrato")')
      ).toBeVisible();
    });
    ...
    test("deve criar chamado com sucesso", async ({ page }) => {
      ...
      await page.fill(
        ...
        "Descricao do chamado de teste para validacao E2E do modulo comercial."
      );
  ...
  test.describe("Triagem", () => {
    test.skip("usuario com permissao deve ver botao de triagem", async ({
      ...
      await expect(
        page.locator('button:has-text("Realizar Triagem")')
      ).toBeVisible();
    });
  ...
  test.describe("Permissoes (RLS)", () => {
    ...
    test.skip("usuario do Comercial deve ver chamados do departamento", async ({
      page,
    }) => {
    ...
    test.skip("usuario de outro departamento nao deve ver chamados do Comercial", async ({
      page,
    }) => {
