...
export async function getSuppliers(filters?: {
  ...
  if (filters?.search) {
    ...
    query = query.or(
      `name.ilike.${search},cnpj.ilike.${search},contact_name.ilike.${search}`
    );
  }
...
export async function createSupplier(data: {
  ...
  const { error } = await supabase.from("accredited_suppliers").insert({
    name: data.name.trim(),
...
export async function deleteSupplier(id: string) {
  ...
  if (count && count > 0) {
    return {
      error:
        "Este fornecedor possui cotações vinculadas e não pode ser excluído. Desative-o em vez disso.",
    };
  }
