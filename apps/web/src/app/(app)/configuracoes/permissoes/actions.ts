...
function getPermissionsForRole(
  ...
  if (
    departmentName &&
    DEPARTMENT_ROLE_PERMISSIONS[departmentName]?.[roleName]
  ) {
 ...
 */
export async function getGlobalRolesWithPermissions(): Promise<
  RoleWithPermissions[]
> {
 ...
 */
export async function getDepartmentsWithRolesAndPermissions(): Promise<
  DepartmentWithRoles[]
> {
  const supabase = await createClient();
