...
export async function getCategories(): Promise<TicketCategory[]> {
  ...
  (ticketCounts || []).forEach((t: { category_id: string | null }) => {
    if (t.category_id) {
      ticketsByCategory[t.category_id] =
        (ticketsByCategory[t.category_id] || 0) + 1;
    }
  });
...
export async function getCategoryStats(): Promise<CategoryStats> {
  ...
  categories.forEach((cat) => {
    stats.byDepartment[cat.department_id] =
      (stats.byDepartment[cat.department_id] || 0) + 1;
  });
...
export async function createCategory(data: {
  ...
  if (error) {
    ...
    if (error.code === "23505") {
      return {
        error: "Já existe uma categoria com este nome neste departamento",
      };
    }
...
export async function deleteCategory(id: string) {
  ...
  if (tickets && tickets.length > 0) {
    return {
      error:
        "Não é possível excluir uma categoria com chamados vinculados. Desative-a em vez de excluir.",
    };
  }
