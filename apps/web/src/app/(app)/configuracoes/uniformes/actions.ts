...
export async function getUniformStats(): Promise<UniformStats> {
  ...
  const stats: UniformStats = {
    ...
    lowStock: uniforms.filter(
      (u) => u.current_stock <= u.min_stock && u.current_stock > 0
    ).length,
...
export async function createUniform(data: {
  ...
  if (insertData.current_stock > 0) {
    await supabase.from("uniform_transactions").insert({
...
export async function deleteUniform(id: string) {
  ...
  if (transactions && transactions.length > 0) {
    return {
      error:
        "Não é possível excluir um uniforme com transações. Zere o estoque em vez de excluir.",
    };
  }
...
export async function adjustUniformStock(
  ...
  if (type === "entrada") {
    ...
    if (newStock < 0) {
      return {
        error: `Estoque insuficiente. Disponível: ${uniform.current_stock}`,
      };
    }
  } else {
    // Ajuste direto
...
export async function canManageUniforms(): Promise<boolean> {
  ...
  const hasRHRole = userRoles?.some((ur: any) => {
    ...
    return (
      role.department_id === rhDept.id &&
      RH_MANAGEMENT_ROLES.includes(role.name)
    );
  });
