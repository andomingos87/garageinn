...
export interface ExecutionDetailsWithAnswers {
  ...
  general_observations: string | null;
  has_non_conformities: boolean | null;
  // Campos de assinatura (supervisão)
...
export async function getExecutionsStats(filters?: {
  ...
  const inProgress =
    data?.filter((e) => e.status === "in_progress").length || 0;
  const withNonConformities =
    data?.filter((e) => e.has_non_conformities).length || 0;
...
export async function getExecutionDetails(
  executionId: string
): Promise<ExecutionDetailsWithAnswers | null> {
  const supabase = await createClient();
...
export async function deleteExecution(executionId: string) {
  ...
  if (!isAdmin) {
    return {
      error: "Acesso negado. Apenas administradores podem excluir execuções.",
    };
  }
