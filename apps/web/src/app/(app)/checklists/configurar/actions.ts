...
export async function checkCanConfigureChecklists(): Promise<boolean> {
  ...
  return userRoles.some((ur) => {
    ...
    if (roleData.is_global && CHECKLISTS_CONFIGURE_PERMISSIONS['global']?.includes(roleData.name)) {
      return true
    }

    // Verificar cargos por departamento (pegar primeiro departamento se existir)
    const departmentName = roleData.department?.[0]?.name
    if (departmentName && CHECKLISTS_CONFIGURE_PERMISSIONS[departmentName]?.includes(roleData.name)) {
      return true
    }
