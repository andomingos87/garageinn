...
export async function getUsers(
  ...
  if (filters?.departmentId) {
    ...
    if (usersInDept) {
      userIdsInDepartment = [
        ...new Set(usersInDept.map((ur: { user_id: string }) => ur.user_id)),
      ];
    }
  }
...
export async function resendInvitation(userId: string) {
  ...
  if (user.deleted_at) {
    return {
      error: "Não é possível reenviar convite para um usuário excluído",
    };
  }
 ...
 */
export async function getUsersStatsExtended(): Promise<
  UsersStats & {
      ...
      if (profile.status === "active") acc.active++;
        ...
        if (profile.invitation_sent_at) {
          if (
            ...
            new Date(profile.invitation_expires_at) < now
          ) {
