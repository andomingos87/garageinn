...
export async function getCities(): Promise<string[]> {
  ...
  const cities = [
    ...new Set(data?.map((u) => u.city).filter((c): c is string => c !== null)),
  ];
  return cities;
}
...
export async function getRegions(): Promise<string[]> {
  ...
  const regions = [
    ...new Set(
      data?.map((u) => u.region).filter((r): r is string => r !== null)
    ),
  ];
  return regions;
}
...
export async function updateUnit(
  ...
  const { error } = await supabase.from("units").update(data).eq("id", unitId);
...
/**
 * Verifica se o usuário atual pode acessar a página de unidades
 * Requer permissão 'units:read' ou 'admin:all'
 */
export async function checkCanAccessUnits(): Promise<boolean> {
  const supabase = await createClient();
  ...
  // Importar funções RBAC inline para evitar dependência circular
...
/**
 * Verifica se o usuário atual pode editar unidades
 * Requer permissão 'units:update' ou 'admin:all'
 */
export async function checkCanEditUnits(): Promise<boolean> {
  const supabase = await createClient();
...
export async function getUnitMetrics(unitId: string): Promise<UnitMetrics> {
  ...
  return {
    ...
    openTickets:
      tickets?.filter((t) => openStatuses.includes(t.status)).length || 0,
    resolvedTickets:
    ...
    checklistsThisMonth:
      checklists?.filter(
        (c) => c.completed_at && new Date(c.completed_at) >= firstDayOfMonth
      ).length || 0,

    nonConformityRate: checklists?.length
      ? Math.round(
          (checklists.filter((c) => c.has_non_conformities).length /
            checklists.length) *
            100
        )
      : 0,
  ...
  return (data || []).map((entry: any) => {
    ...
    if (entry.action === "unit.update" && entry.old_data && entry.new_data) {
      ...
      changes.push({
        field: "Criação",
        old_value: null,
        new_value: "Unidade criada",
      });
    } else if (entry.action === "unit.delete") {
      changes.push({
        field: "Exclusão",
        old_value: null,
        new_value: "Unidade excluída",
      });
    }
...
export async function previewSupervisorLinks(): Promise<SupervisorLinkPreview> {
  ...
  profiles?.forEach((p) => {
    profileMap.set(p.full_name.toLowerCase().trim(), {
      id: p.id,
      name: p.full_name,
    });
  });
...
export async function linkSupervisorsFromImport(): Promise<{
  ...
  for (const unit of unitsWithSupervisor || []) {
    ...
    const { error: insertError } = await supabase.from("user_units").insert({
      user_id: profile.id,
      unit_id: unit.id,
      is_coverage: true,
    });
