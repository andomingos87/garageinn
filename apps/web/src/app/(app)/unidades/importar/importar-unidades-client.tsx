...
export default function ImportarUnidadesClient() {
  ...
  const handleParse = useCallback(
    async (data: Record<string, string>[], headers: string[]) => {
      setError(null);

      // Verificar headers esperados
      const hasNameColumn = headers.some((h) =>
        h.toUpperCase().includes("UNIDADE")
      );
      const hasAddressColumn = headers.some((h) =>
        h.toUpperCase().includes("ENDERE")
      );
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        for (const [csvCol, fieldName] of Object.entries(CSV_COLUMN_MAP)) {
          ...
          const csvKey = Object.keys(row).find(
            (k) =>
              k.toUpperCase().replace(/[^A-Z]/g, "") ===
              csvCol.toUpperCase().replace(/[^A-Z]/g, "")
          );
  ...
  const handleConfirmImport = () => {
    startTransition(async () => {
      ...
      try {
        ...
        setError(
          err instanceof Error ? err.message : "Erro ao importar unidades"
        );
      }
    });
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2 text-sm">
        <div
          className={`flex items-center gap-2 ${step === "upload" ? "text-primary font-medium" : "text-muted-foreground"}`}
        >
          <div
        ...
        <div
          className={`flex items-center gap-2 ${step === "preview" ? "text-primary font-medium" : "text-muted-foreground"}`}
        >
          <div
            className={`w-6 h-6 rounded-full flex items-center justify-center text-xs ${step === "preview" ? "bg-primary text-primary-foreground" : "bg-muted"}`}
          >
            2
          </div>
          Preview
        ...
        <div className="w-8 h-px bg-border" />
        <div
          className={`flex items-center gap-2 ${step === "result" ? "text-primary font-medium" : "text-muted-foreground"}`}
        >
          <div
            ...
            <CardContent className="space-y-4">
              ...
              <div className="text-sm text-muted-foreground space-y-2">
                ...
                <ul className="list-disc list-inside space-y-1 text-xs">
                  ...
                  <li>
                    Colunas opcionais: VAGAS, CNPJ, SUPERVISOR, ADMINISTRADORA,
                    BAIRRO, REGIÃO, CIDADE, ESTADO
                  </li>
                </ul>
              </div>
            </CardContent>
