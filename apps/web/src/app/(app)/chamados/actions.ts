...
export async function getHubTickets(filters?: HubTicketFilters) {
  ...
  let query = supabase
    ...
    .select(
      "id, ticket_number, title, status, priority, created_at, department_id, department_name, unit_id, unit_name, unit_code, assigned_to_id, assigned_to_name",
      { count: "exact" }
    )
...
export async function getHubStats(): Promise<HubStats> {
  ...
  const [triageResult, inProgressResult, resolvedResult, totalResult] =
    await Promise.all([
      supabase
        .from("tickets")
        .select("*", { count: "exact", head: true })
        .in("status", triageStatuses),
      supabase
        .from("tickets")
        .select("*", { count: "exact", head: true })
