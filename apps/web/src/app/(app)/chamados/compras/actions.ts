...
export async function checkIsGerente(): Promise<boolean> {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const hasGerenteRole = userRoles?.some((ur: any) => 
    ur.role?.name === 'Gerente'
  )
...
export async function changeTicketStatus(
  ...
  if ((isAdmin || isGerente) && adminOverrideStatuses.includes(newStatus) && !finalStatuses.includes(ticket.status)) {
    // Permitir transição
  } else {
...
export async function canTriageTicket(): Promise<boolean> {
  ...
  const hasTriageRole = userRoles?.some((ur: any) => {
    ...
    if (['Desenvolvedor', 'Administrador', 'Diretor', 'Gerente'].includes(role.name)) return true
