    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          ...
          <DialogDescription>
            <span className="font-medium text-foreground">#{ticketNumber}</span>
            {" - "}
            {ticketTitle}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="bg-muted/50 rounded-lg p-3 text-sm space-y-2">
          {maintenanceType && (
            <div className="flex items-center gap-2">
              ...
              <span className="font-medium">
                {MAINTENANCE_TYPES[
                  maintenanceType as keyof typeof MAINTENANCE_TYPES
                ] || maintenanceType}
              </span>
            </div>
          )}
          {locationDescription && (
            <div className="flex items-center gap-2">
              ...
              <span className="font-medium">{locationDescription}</span>
            </div>
          )}
          {perceivedUrgency && (
            <div className="flex items-center gap-2">
              ...
              <Badge
                variant="outline"
                className={urgencyLabels[perceivedUrgency]?.color}
              >
          ...
          <div className="space-y-2">
            ...
            {dueDate && (
              <p className="text-xs text-muted-foreground">
                Previsão:{" "}
                {format(new Date(dueDate + "T12:00:00"), "EEEE, dd 'de' MMMM", {
                  locale: ptBR,
                })}
              </p>
            )}
                ...
                {selectedPriority && (
                  <div className="flex items-center gap-2">
                    <selectedPriority.icon
                      className={cn("h-4 w-4", selectedPriority.color)}
                    />
                    <span>
                      Prioridade: <strong>{selectedPriority.label}</strong>
                    </span>
                  </div>
                )}
                {selectedMember && (
                  <div className="flex items-center gap-2">
                    ...
                    <span>
                      Responsável: <strong>{selectedMember.full_name}</strong>
                    </span>
                  </div>
                )}
                {dueDate && (
