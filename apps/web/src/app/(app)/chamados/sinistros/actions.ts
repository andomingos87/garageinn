...
// Note: getUserUnits, getUserFixedUnit, and UserUnit should be imported directly from '@/lib/units'
// Cannot re-export non-async functions from "use server" files
...
export async function createClaimTicket(formData: FormData) {
  ...
  const location_description = formData.get("location_description") as
    | string
    | null;
  const police_report_number = formData.get("police_report_number") as
    | string
    | null;
  ...
  const { error: detailsError } = await supabase
    ...
    .insert({
      ...
      third_party_plate: has_third_party
        ? third_party_plate?.toUpperCase()
        : null,
    });
...
export async function canTriageClaimTicket(): Promise<boolean> {
  ...
  // Verificar se tem cargo de triagem no departamento de Sinistros
  const triageRoles = ["Supervisor", "Gerente", "Coordenador", "Analista"];
  // Cargos que podem triar chamados de qualquer departamento
  ...
  return userRoles.some((ur) => {
    ...
    // Se é um cargo de triagem dentro do departamento de Sinistros
