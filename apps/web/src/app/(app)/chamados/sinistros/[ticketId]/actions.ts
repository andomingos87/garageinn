...
export async function addClaimCommunication(
  ...
  const { error } = await supabase.from("claim_communications").insert({
...
export async function addClaimPurchaseItem(
  ...
  // Criar item
  const { error } = await supabase.from("claim_purchase_items").insert({
...
export async function addClaimQuotation(
  ...
  // Criar cotação
  const { error } = await supabase.from("claim_purchase_quotations").insert({
    claim_purchase_id: purchaseId,
...
export async function approveClaimPurchase(
  ...
  if (quotationsCount < 2 && decision === "approved") {
    return {
      error: "São necessárias pelo menos 2 cotações para aprovar a compra",
    };
  }
  ...
  const { error } = await supabase
    ...
    .update({
      ...
      rejection_reason:
        decision === "rejected" ? notes || "Rejeitada pelo gerente" : null,
    })
    .eq("id", purchaseId);
...
export async function createMaintenanceFromClaim(ticketId: string) {
  ...
  if (claimDetails.related_maintenance_ticket_id) {
    return {
      error: "Já existe um chamado de manutenção vinculado a este sinistro",
    };
  }
...
Este chamado foi criado automaticamente pelo sistema de sinistros.
  ...
  if (ticketError || !maintenanceTicket) {
    ...
    return {
      error: ticketError?.message || "Erro ao criar chamado de manutenção",
    };
  }
