...
export function ClaimCommunications({
  ...
  const getChannelLabel = (channelValue: string) => {
    return (
      COMMUNICATION_CHANNELS.find((c) => c.value === channelValue)?.label ||
      channelValue
    );
  };
              ...
              <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                ...
                <DialogContent className="sm:max-w-[500px]">
                  <DialogHeader>
                    ...
                    <DialogDescription>
                      Registre uma comunicação realizada com o cliente sobre
                      este sinistro.
                    </DialogDescription>
                  </DialogHeader>
                      ...
                      <Select value={channel} onValueChange={setChannel}>
                        ...
                        <SelectContent>
                          {COMMUNICATION_CHANNELS.map((ch) => {
                            const Icon =
                              channelIcons[ch.value] || MessageSquare;
                            return (
          ...
          {sortedCommunications.length === 0 ? (
            ...
            <div className="space-y-3 max-h-[400px] overflow-y-auto">
              {sortedCommunications.map((comm) => {
                ...
                const colorClass =
                  channelColors[comm.channel] || channelColors.outro;
                      ...
                      <div className="flex items-start justify-between gap-2">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          <span className="text-xs text-muted-foreground">
                            {format(
                              new Date(comm.communication_date),
                              "dd/MM/yyyy 'às' HH:mm",
                              { locale: ptBR }
                            )}
                          </span>
                      ...
                      <div className="flex items-center justify-between mt-2 pt-2 border-t border-border/50">
                        <div className="flex items-center gap-2">
                          <Avatar className="h-5 w-5">
                            <AvatarImage
                              src={comm.creator?.avatar_url || undefined}
                            />
                        ...
                        {comm.next_contact_date && (
                          <div className="flex items-center gap-1 text-xs text-muted-foreground">
                            ...
                            <span>
                              Próximo:{" "}
                              {format(
                                new Date(comm.next_contact_date),
                                "dd/MM/yyyy",
                                { locale: ptBR }
                              )}
                            </span>
