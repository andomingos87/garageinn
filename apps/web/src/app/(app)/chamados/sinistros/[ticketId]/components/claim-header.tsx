...
export function ClaimHeader({ ticket }: ClaimHeaderProps) {
  ...
  const occurrenceLabel =
    OCCURRENCE_TYPES.find((t) => t.value === occurrenceType)?.label ||
    occurrenceType;
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {occurrenceType && (
              <Badge
                variant="outline"
                className="gap-1 bg-orange-50 text-orange-700 border-orange-200 dark:bg-orange-950/30 dark:text-orange-400 dark:border-orange-800"
              >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 bg-muted/30 rounded-lg">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="min-w-0">
            ...
            <p className="text-sm font-medium truncate">
              {ticket.category?.name || "Não definida"}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="min-w-0">
            ...
            <p className="text-sm font-medium truncate">
              {ticket.unit
                ? `${ticket.unit.name} (${ticket.unit.code})`
                : "Não definida"}
            </p>
          </div>
        </div>
            ...
            {claimDetails.occurrence_date && (
              <div className="flex items-center gap-2 text-muted-foreground">
                ...
                <span>
                  Ocorrência:{" "}
                  {format(
                    new Date(claimDetails.occurrence_date),
                    "dd/MM/yyyy 'às' HH:mm",
                    { locale: ptBR }
                  )}
                </span>
