...
> = {
  ...
  triaged: {
    icon: FileSearch,
    color: "text-primary",
    label: "Sinistro triado",
  },
  // Comunicações
  communication_telefone: {
    icon: Phone,
    color: "text-blue-500",
  ...
  communication_presencial: {
    icon: Users,
    color: "text-orange-500",
    label: "Contato presencial",
  },
  communication_outro: {
    icon: MessageSquare,
    color: "text-gray-500",
  ...
  const renderTimelineItem = (item: TimelineItem) => {
    if (item.type === "history") {
      ...
      const {
        ...
        color,
        description,
      } = getActionDescription(historyItem);
            ...
            <div className="flex items-start justify-between gap-2">
              <div className="flex items-center gap-2 min-w-0">
                <Avatar className="h-5 w-5">
                  <AvatarImage
                    src={historyItem.user?.avatar_url || undefined}
                  />
    ...
    if (item.type === "purchase") {
      ...
      return (
        <div key={item.id} className="relative flex gap-4 pl-10">
          <div
            className={`absolute left-0 p-1.5 bg-background border rounded-full ${config.color}`}
          >
            <Icon className="h-3 w-3" />
          </div>
    ...
    <Card>
      ...
      <CardContent>
        <div className="relative">
          ...
          <div className="space-y-4">{sortedItems.map(renderTimelineItem)}</div>
        </div>
      </CardContent>
    </Card>
  );
