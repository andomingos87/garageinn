...
export async function createRHTicket(formData: FormData) {
  ...
  const specific_fields: Record<string, string | number | null> = {};

  // Se for uniforme, extrair dados do uniforme
  if (rh_type === "uniform") {
    specific_fields.uniform_id = formData.get("uniform_id") as string | null;
    specific_fields.quantity =
...
export async function getRHTicketDetails(ticketId: string) {
  ...
  let uniform = null;
  if (
    rhDetails?.rh_type === "uniform" &&
    rhDetails.specific_fields?.uniform_id
  ) {
...
export async function canTriageRHTicket(): Promise<boolean> {
  ...
  const hasTriageRole = userRoles?.some((ur: any) => {
    ...
    if (GLOBAL_TRIAGE_ROLES.includes(role.name)) return true;

    // Se é um cargo de triagem dentro do departamento de RH
