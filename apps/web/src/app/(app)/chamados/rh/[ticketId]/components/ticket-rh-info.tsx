...
export function TicketRHInfo({ ticket }: TicketRHInfoProps) {
  ...
  const stockStatus =
    rh_details?.rh_type === "uniform"
      ? getStockStatus(uniform, requestedQuantity)
      : null;
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="space-y-1">
            ...
            <div className="font-medium">
              {(rh_details?.rh_type && RH_TYPE_LABELS[rh_details.rh_type]) ||
                rh_details?.rh_type}
            </div>
          </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="grid gap-1 flex-1">
                    <div className="flex items-center justify-between">
                      <span
                        className={`font-semibold ${stockStatus.textClass}`}
                      >
                        {stockStatus.label}
                      </span>
                    ...
                    {stockStatus.status === "insufficient" && (
                      <div className="text-xs text-red-600 mt-1">
                        ⚠️ Não é possível entregar este uniforme sem reposição
                        do estoque
                      </div>
                    )}
                    {stockStatus.status === "low" && (
