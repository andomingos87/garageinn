...
import { Play, CheckCircle, XCircle, Ban, Settings } from "lucide-react";
...
export function RHTicketActions({
  ...
  const showUniformDelivery =
    isUniformTicket && currentStatus === "in_progress" && canTriage;
  ...
  if (canManage) {
    ...
    if (currentStatus === "in_progress" && !isUniformTicket)
      allowedTransitions.push("resolved");
    if (currentStatus === "resolved") allowedTransitions.push("closed");
    if (["awaiting_triage", "in_progress"].includes(currentStatus))
      allowedTransitions.push("denied");
  }
  ...
  const handleDeny = () => {
    ...
    startTransition(async () => {
      const result = await changeRHTicketStatus(
        ticket.id,
        "denied",
        denyReason
      );
        ...
        <CardContent className="space-y-3">
          ...
          {showUniformDelivery && ticket.uniform && (
            <UniformDeliveryDialog
              ...
              uniformSize={ticket.uniform.size ?? null}
