...
export function ComercialForm({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (formData.client_name)
      data.set("client_name", formData.client_name.trim());
    ...
    if (formData.client_email)
      data.set("client_email", formData.client_email.trim());

    // Contrato
    if (formData.contract_value) {
      data.set(
        "contract_value",
        parseCurrency(formData.contract_value).toString()
      );
    }
    if (formData.contract_start_date)
      data.set("contract_start_date", formData.contract_start_date);
    if (formData.contract_end_date)
      data.set("contract_end_date", formData.contract_end_date);
    if (formData.proposal_deadline)
      data.set("proposal_deadline", formData.proposal_deadline);

    // Informacoes adicionais
    if (formData.competitor_info)
      data.set("competitor_info", formData.competitor_info.trim());
    if (formData.negotiation_notes)
      data.set("negotiation_notes", formData.negotiation_notes.trim());
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground">
            Preencha as informacoes do chamado
          </p>
        </div>
      </div>
      ...
      {showUnitWarning && (
        <div className="bg-amber-50 text-amber-800 px-4 py-3 rounded-lg border border-amber-200 dark:bg-amber-900/20 dark:text-amber-200 dark:border-amber-800">
          <strong>Atencao:</strong> Voce nao possui unidades atribuidas. Entre
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Informacoes do cliente relacionado ao chamado
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid gap-4 md:grid-cols-2">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  handleChange("proposal_deadline", e.target.value)
                }
                disabled={isPending}
              />
            </div>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  handleChange("contract_end_date", e.target.value)
                }
                disabled={isPending}
              />
            </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Informacoes complementares sobre a negociacao
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="space-y-2">
          ...
          <div className="space-y-2">
            ...
            <Textarea
              ...
              onChange={(e) =>
                handleChange("negotiation_notes", e.target.value)
              }
              disabled={isPending}
              rows={3}
            />
