...
function getUserPermissions(roles: UserRole[]): Permission[] {
  ...
  for (const role of roles) {
    ...
    if (role.department_name) {
      const deptPerms =
        DEPARTMENT_ROLE_PERMISSIONS[role.department_name]?.[role.role_name];
      if (deptPerms) {
...
const EXPECTED_PERMISSIONS: Record<
  string,
  { minPerms?: Permission[]; hasAdmin?: boolean }
> = {
  // Global roles
  ...
  // Operações
  "Operações|Manobrista": {
    minPerms: ["tickets:read", "tickets:create", "checklists:read"],
  },
  "Operações|Encarregado": {
    minPerms: ["tickets:read", "tickets:approve", "units:read"],
  },
  "Operações|Supervisor": { minPerms: ["tickets:read", "tickets:approve"] },
  "Operações|Gerente": {
    minPerms: ["tickets:read", "tickets:triage", "checklists:configure"],
  },
  // Compras e Manutenção
  "Compras e Manutenção|Assistente": { minPerms: ["tickets:read"] },
  "Compras e Manutenção|Comprador": {
    minPerms: ["tickets:read", "tickets:execute"],
  },
  "Compras e Manutenção|Gerente": {
    minPerms: ["tickets:read", "tickets:triage", "tickets:execute"],
  },
  // Financeiro
  "Financeiro|Auxiliar": { minPerms: ["tickets:read"] },
  "Financeiro|Assistente": { minPerms: ["tickets:read"] },
  "Financeiro|Analista Júnior": {
    minPerms: ["tickets:read", "tickets:execute"],
  },
  "Financeiro|Analista Pleno": {
    minPerms: ["tickets:read", "tickets:approve"],
  },
  "Financeiro|Analista Sênior": {
    minPerms: ["tickets:read", "tickets:approve"],
  },
  "Financeiro|Supervisor": { minPerms: ["tickets:read", "tickets:triage"] },
  "Financeiro|Gerente": {
    minPerms: ["tickets:read", "tickets:triage", "settings:read"],
  },
  // TI
  ...
  // Comercial
  "Comercial|Gerente": {
    minPerms: ["units:read", "tickets:read", "settings:read"],
  },
  // Auditoria
  "Auditoria|Auditor": { minPerms: ["tickets:read", "checklists:read"] },
  "Auditoria|Gerente": {
    minPerms: ["tickets:read", "tickets:approve", "checklists:configure"],
  },
  // Sinistros
  "Sinistros|Supervisor": {
    minPerms: ["tickets:read", "tickets:execute", "tickets:approve"],
  },
  "Sinistros|Gerente": {
    minPerms: ["tickets:read", "tickets:triage", "settings:read"],
  },
...
for (const role of DATABASE_ROLES) {
  const key = role.department_name
...
console.log("=".repeat(60));
console.log(
  `RESULTS: ${passed} passed, ${failed} failed, ${DATABASE_ROLES.length} total`
);
console.log("=".repeat(60));
...
const specificTests = [
  ...
  },
  {
    ...
    check: (perms: Permission[]) =>
      perms.includes("users:read") && perms.includes("users:create"),
  },
  {
    ...
    check: (perms: Permission[]) =>
      perms.includes("tickets:read") &&
      perms.includes("tickets:execute") &&
      perms.includes("tickets:approve"),
  },
  {
    ...
    check: (perms: Permission[]) =>
      perms.includes("tickets:read") && perms.includes("tickets:execute"),
  },
];
